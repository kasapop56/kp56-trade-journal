  ...
  try {
    if (payload.event === 'trade_closed') {
      const row = {
        ...
        // closed by hand shows magic 0 but open_magic 56). Spread-in only when
        // the EA actually sent it: including the key unconditionally would 500
        // every upsert until the open_magic column migration has been run.
        ...(payload.open_magic !== undefined ? { open_magic: payload.open_magic } : {}),
